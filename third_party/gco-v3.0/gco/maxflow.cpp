...
*/
#define MF_TERMINAL ( (arc *) 1 )		/* to terminal */
	...
	else
	{
		...
		for (a0=i->first; a0; a0=a0->next)
		{
			...
			if (!j->is_sink && (a=j->parent))
			{
				...
				if (a!=MF_TERMINAL && a!=ORPHAN && a->head==i)
				{
	...
	void Graph<captype,tcaptype,flowtype>::test_consistency(node* current_node)
{
	...
	for (i=nodes; i<node_last; i++)
	{
		...
		if (i->parent && i->parent!=ORPHAN && i->parent!=MF_TERMINAL)
		{
	...
	void Graph<captype,tcaptype,flowtype>::Copy(Graph<captype, tcaptype, flowtype>* g0)
{
	...
	for (i=nodes; i<node_last; i++)
	{
		...
		if (i->parent && i->parent!=MF_TERMINAL && i->parent!=ORPHAN) i->parent = (arc*)((char*)arcs + (((char*)i->parent) - ((char*)g0->arcs)));
