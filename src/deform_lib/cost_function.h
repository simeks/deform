...
#if DF_USE_ISPC
    // This header is generated by ISPC at build time
    #include <ispc_lib.h>
#endif
...
struct GradientSSDFunction : public SubFunction
{
    GradientSSDFunction(const stk::VolumeHelper<T>& fixed,
                        const stk::VolumeHelper<T>& moving,
                        const int sigma) :
        _fixed(stk::sobel(filters::gaussian_filter_3d(fixed, sigma))),
        _moving(stk::sobel(filters::gaussian_filter_3d(moving, sigma)))
    {}
